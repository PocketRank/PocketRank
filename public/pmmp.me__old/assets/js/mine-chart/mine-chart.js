/**
 * @link https://github.com/McbeRank/MineChart
 * @author SOLO
 */
(function () {
    'use strict';
    ...
    MineChart.prototype.load = function (name) {
        return d3.json("/api/getPlayerCounts/" + name).then(data => new Promise(function (resolve) {
            resolve(d3.csvParse(data['result'].split('\n').join("\n")));
        }));
    }
    ...
    MineChart.prototype.timeInterval = function (interval) {
        ...
        // update global min, max
        that.min = d3.min(Object.values(that.data), function (data) {
            return data.min;
        });
        that.max = d3.max(Object.values(that.data), function (data) {
            return data.max;
        });
    ...
    MineChart.prototype.updateYAxis = function (transition = false) {
        ...
        min = Math.max(0, Math.floor((min - 1) / step) * step);
